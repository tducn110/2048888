...
export function isOfflineModeEnabled(): boolean {
  ...
  try {
    return window.location.search.includes('offline=true') ||
      window.sessionStorage.getItem('wink_offline_mode') === 'true';
  } catch {
    return false;
  }
}
...
export function useWinkIntegration(): WinkIntegration {
  ...
  const [sdk, setSdk] = useState<WinkSDK | null>(typeof window !== 'undefined' && !isOffline ? window.Wink || null : null);
  const [status, setStatus] = useState<WinkStatus>(isOffline ? 'standalone' : (sdk?.status ?? 'connecting'));
  const [isReady, setIsReady] = useState(isOffline);
  ...
  useEffect(() => {
    ...
    void resolveGlobalWink().then((resolvedSdk) => {
      ...
      if (resolvedSdk) {
        ...
        try {
          cleanups.push(resolvedSdk.on('pause', () => setHostPaused(true)));
          cleanups.push(resolvedSdk.on('resume', () => setHostPaused(false)));
          cleanups.push(resolvedSdk.on('mute', () => setParentMuted(true)));
          cleanups.push(resolvedSdk.on('unmute', () => setParentMuted(false)));
          cleanups.push(resolvedSdk.on('locale', (nextLocale: string) => setLocale(nextLocale)));
        } catch (e) {
  ...
  const refreshLeaderboard = useCallback(async () => {
    ...
    if (!currentSdk || !currentSdk.can('getLeaderboard')) {
      setLeaderboard([]);
      return;
    }
    ...
    async (input: {
      ...
      try {
        ...
        return {
          ...
          previousBest: response.previousBest,
        };
      } catch (err: unknown) {
  ...
  const phase: WinkPhase =
    ...
    status === 'online' ? (sdk?.player?.isGuest ? 'ready_anonymous' : 'ready_authenticated') :
    status === 'connected' ? 'ready_anonymous' :
    'ready_anonymous';
