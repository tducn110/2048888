/**
 * Contract tests for the 2048 game's Wink adapter.
 *
 ...
 *
 * 2048-specific cases added below the standard contract tests:
 * - revive does NOT emit a second completion on the same round
 * - score is independent from completion (double-score button does not complete)
 */
...
// ---------------------------------------------------------------------------
// Standard contract tests (required by the handoff verifier)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 2048-specific contract tests
// ---------------------------------------------------------------------------

describe('2048 revive — same round, completion still fires exactly once', () => {
  it('revive step does not trigger completion; only final game-over does', () => {
    const { api } = install(AUTHENTICATED);
    const wink = new WinkGameIntegration();

    // Round starts when first tile moves
    const round = wink.startRound();

    // Simulate: player hits game-over, chooses to revive
    // (revive does NOT call completeRound — the round is still active)
    // ...player continues playing and hits game-over again, declines revive

    // Only now do we complete:
    expect(wink.completeRound(round, { metadata: { outcome: 'lost' } })).toBe(true);
    expect(api.complete).toHaveBeenCalledTimes(1);

    // Calling again (e.g. from a second code path) is a no-op:
    expect(wink.completeRound(round)).toBe(false);
    expect(api.complete).toHaveBeenCalledTimes(1);
  });
});

describe('2048 x2 score — score submission is independent from completion', () => {
  it('doubling the score before submit is fine; score and complete are independent', async () => {
    const { api } = install(AUTHENTICATED);
    const wink = new WinkGameIntegration();

    const round = wink.startRound();

    // Player chose x2 bonus then the game records final game-over
    // Score submitted first, then complete — order does not matter, neither
    // triggers the other:
    await wink.submitFinalScore({ score: 2048 * 2, playTime: 180, gameMode: 'classic' });
    wink.completeRound(round, { metadata: { outcome: 'lost', doubled: true } });

    expect(api.submitScore).toHaveBeenCalledTimes(1);
    expect(api.complete).toHaveBeenCalledTimes(1);
  });

  it('anonymous x2 still surfaces CAPABILITY_DENIED; completion still works', async () => {
    const { api } = install(ANONYMOUS);
    const wink = new WinkGameIntegration();

    const round = wink.startRound();

    await expect(wink.submitFinalScore({ score: 2048 })).rejects.toMatchObject({
      code: 'CAPABILITY_DENIED',
    });

    // Completion is a separate capability and is allowed for anonymous:
    expect(wink.completeRound(round)).toBe(true);
    expect(api.complete).toHaveBeenCalledTimes(1);
  });
});

describe('2048 new-game reset — each game is its own round', () => {
  it('two consecutive games have distinct round ids', () => {
    install(AUTHENTICATED);
    const wink = new WinkGameIntegration();

    const round1 = wink.startRound();
    wink.completeRound(round1);

    const round2 = wink.startRound();
    wink.completeRound(round2);

    expect(round1.roundId).not.toBe(round2.roundId);
  });

  it('completing round1 does not affect round2 completion', () => {
    const { api } = install(AUTHENTICATED);
    const wink = new WinkGameIntegration();
    ...
    wink.completeRound(round1);
    expect(wink.completeRound(round2)).toBe(true);
